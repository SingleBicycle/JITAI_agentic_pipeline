...
def clusters_by_kmeans(
    ...
    if len(entities) == 1:
        ...
        merged_entity = {
            ...
            "roles": sorted(set([entity.get("role", "unknown")])),
            "descriptions": entity["description"],
            "timestamps": entity["timestamps"],
            "frame_indices": entity["frame_indices"],
            "durations": [entity["duration"]],
            "events": [entity["event"]],
        }
        return [merged_entity], {entity["id"]: merged_entity["id"]}
    ...
    if not np.isfinite(embeddings).all():
        raise ValueError("Entity embeddings contain non-finite values after sanitization.")

    if np.allclose(embeddings, 0.0):
        merged_entity, id_map = merge_entity_group(entities)
        return [merged_entity], id_map
    ...
    for cluster in clusters:
        ...
        names = sorted(set([entity["name"] for entity in cluster]))
        roles = sorted(set([entity.get("role", "unknown") for entity in cluster]))
        ...
        role_counter = Counter([entity.get("role", "unknown") for entity in cluster])
        merged_entity = {
            "id": new_id,
            "names": names,
            "role": role_counter.most_common(1)[0][0],
            "roles": roles,
    ...
    return merged_entities, id_map


def merge_entity_group(cluster: list[dict]) -> tuple[dict, dict]:
    descriptions = [description for entity in cluster for description in entity["description"]]
    names = sorted(set([entity["name"] for entity in cluster]))
    roles = sorted(set([entity.get("role", "unknown") for entity in cluster]))
    timestamps = sorted(set([timestamp for entity in cluster for timestamp in entity["timestamps"]]))
    frame_indices = sorted(set([frame_index for entity in cluster for frame_index in entity["frame_indices"]]))
    duration = sorted([entity["duration"] for entity in cluster], key=lambda x: x[0])
    events = []
    for entity in cluster:
    ...
    role_counter = Counter([entity.get("role", "unknown") for entity in cluster])
    primary_name = names[0] if names else cluster[0]["name"]
    new_id = compute_mdhash_id(primary_name + "".join(descriptions), prefix="Entity-")
    merged_entity = {
        "id": new_id,
        "names": names,
        "role": role_counter.most_common(1)[0][0],
    ...
    id_map = {entity["id"]: new_id for entity in cluster}
    return merged_entity, id_map


def rewrite_relations(relations: list[dict], id_map: dict) -> list[dict]:
    deduped_relations = {}
    for relation in relations:
        entity1 = id_map.get(relation["entity1"], relation["entity1"])
        entity2 = id_map.get(relation["entity2"], relation["entity2"])
        if entity1 == entity2:
            continue
        description = relation["description"]
        relation_id = compute_mdhash_id(entity1 + entity2 + description, prefix="Relation-")
        deduped_relations[relation_id] = {
            "id": relation_id,
            "entity1": entity1,
            "entity2": entity2,
            "description": description,
        }
    return list(deduped_relations.values())


def merge_entities_role_aware(
    entities: list[dict],
    relations: list[dict],
    embedding_model: BaseEmbeddingModel,
    file_path: str,
    ...
    role_locked_groups = defaultdict(list)
    cluster_candidates = []
    role_locked_prefixes = ("child_", "caregiver_", "clinician_", "other_child_", "other_adult_")

    for entity in entities:
        name = entity.get("name", "")
        role = entity.get("role", "unknown")
        if name.startswith(role_locked_prefixes) or role in {
            "child",
            "caregiver",
            "clinician",
            "other_child",
            ...
            role_locked_groups[(role, name)].append(entity)
        else:
            cluster_candidates.append(entity)

    merged_entities = []
    id_map = {}

    for cluster in role_locked_groups.values():
        merged_entity, cluster_id_map = merge_entity_group(cluster)
        merged_entities.append(merged_entity)
        id_map.update(cluster_id_map)

    clustered_entities, clustered_id_map = clusters_by_kmeans(
    ...
    merged_entities.extend(clustered_entities)
    id_map.update(clustered_id_map)

    merged_relations = rewrite_relations(relations, id_map)
...
def validate_entities_relations(
    ...
    for entity in entities:
        ...
        if "Entity_role" in entity and not isinstance(entity["Entity_role"], str):
            event_retries[event_idx] += 1
            raise ValueError("Entity role must be a string.")
...
def format_entities_and_relations(
    ...
    for entity in entities:
        ...
        formatted_entity = {
            ...
            "role": clean_str(entity.get("Entity_role", "unknown")).lower(),
...
def batch_generate_entities_and_relations(
    ...
    while remaining_indices:
        ...
        batch_inputs = []
        entity_prompt_name = global_config.get(
            "entity_prompt_name", "entity_relation_extraction"
        )
        entity_prompt = PROMPTS.get(
            entity_prompt_name, PROMPTS["entity_relation_extraction"]
        )
...
def extract_entities_and_relations(
    ...
    merged_entities, merged_relations = merge_entities_role_aware(
