...
PROMPTS["generate_description_dyadic"] = """
You are an expert in video understanding for caregiver-child interaction analysis.
Your task is to describe the video segment as a coherent event while preserving the identity of each visible person.

Instructions:
- Treat each visible person as a distinct individual.
- If one person appears to be the child and another appears to be the caregiver, keep them separate and describe their actions separately.
- Do not merge multiple humans into one generic "person".
- Focus on observable behavior only: posture, movement, proximity, gaze direction when visible, object use, touch, handover, blocking, approach, withdrawal, and co-regulation behavior.
- Avoid diagnosis, internal states, or intentions unless they are directly observable.
- Keep the description chronological, concise, and grounded in the frames.

Return one fluent paragraph no longer than 300 words.
"""

PROMPTS["entity_relation_extraction_role_aware"] = """
You are a vision-based information extraction expert for caregiver-child interaction analysis.
Your task is to analyze a sequence of frames and extract distinct visible entities and their relationships.

Rules:
1. Keep each visible person separate. Never collapse multiple humans into one entity.
2. If possible, assign stable names such as `child_1`, `caregiver_1`, `clinician_1`, `other_child_1`, `other_adult_1`.
3. Use `Entity_role` from this set when possible:
   `child`, `caregiver`, `clinician`, `other_child`, `other_adult`, `object`, `unknown`
4. For non-human items, use concise names such as `toy_block`, `tablet`, `chair`, `book`, `cup`.
5. Describe only visible evidence. Do not infer diagnosis or emotion as fact.
6. Record the frame indices where each entity is visible in `Index`.
7. Extract meaningful visual relationships such as proximity, physical contact, handover, joint attention, blocking, guiding, sitting beside, looking toward, moving away from.
...
{{
  "Entities": [
    {{
      "Entity_name": "child_1",
      "Entity_role": "child",
      "Entity_description": "young child in a blue shirt seated on the floor near a toy bin, repeatedly reaching toward the caregiver",
      "Index": [0, 1, 3, 5]
    }}
  ],
  "Relations": [
    {{
      "Entity1": "caregiver_1",
      "Entity2": "child_1",
      "Relation_description": "caregiver kneels close to the child and extends an object toward the child"
    }}
  ]
}}
```
...
PROMPTS["summary_and_answer_open"] = """
You are an advanced AI system for open-ended video analysis.
You will receive retrieved video segments organized by timestamps and descriptions.
Answer the user query in grounded natural language and return clean JSON only.

Instructions:
1. Base the answer only on the provided evidence.
2. If evidence is incomplete, say so in `limitations`.
3. Quote the most important evidence windows in `evidence`.
...
{{
  ...
  "evidence": [
    "12.0s-21.0s: caregiver kneels beside the child and hands over a toy"
  ],
  "confidence": 0.0,
  "limitations": [
    "audio was not used"
...
PROMPTS["checkframe_and_answer_open"] = """
You are an advanced AI system for open-ended video analysis.
You are given sampled frames from a candidate time window and a user query.
Answer using only what is visually supported by the frames and return clean JSON only.
...
{{
  "answer": "short grounded answer",
  "evidence": [
    "caregiver is within arm's reach of the child",
    "child is holding a red object"
  ],
  "confidence": 0.0,
  "limitations": [
    "speech content cannot be verified from frames alone"
  ]
}}
"""

PROMPTS["jitai_window_assessment"] = """
You are assisting with caregiver-facing just-in-time intervention support for a child interaction session.
You are given sampled video frames and structured context for a single time window.
Your task is to summarize observable interaction state and estimate whether caregiver support may be needed soon.

Important:
- This is NOT diagnosis.
- Use only observable evidence.
- If uncertain, say `unclear`.
- Keep role identity stable: `child_1`, `caregiver_1`, etc.
...
{{
  "child_state": {{
    ...
    "affect": "positive|neutral|distressed|unclear",
    "attention_target": "child_1 focuses on caregiver_1 / toy / away / unclear",
    "state_summary": "one sentence",
    "observable_cues": ["cue 1", "cue 2"]
  }},
  "caregiver_state": {{
    "proximity": "close|near|far|unclear",
    "attention_to_child": "high|medium|low|unclear",
    "response_latency_estimate_sec": 0.0,
    "support_actions": ["action 1", "action 2"]
  }},
  "interaction_state": {{
    "shared_attention": "present|partial|absent|unclear",
    "turn_taking": "present|limited|absent|unclear",
    "escalation_trend": "up|flat|down|unclear"
  }},
  "risk_flags": ["flag 1"],
  "possible_triggers": ["trigger 1"],
  "recommended_intervention_tags": ["co_regulation", "reduce_demands"],
  "evidence": ["specific observation 1", "specific observation 2"],
  "confidence": 0.0,
  "uncertainty_notes": ["what remains uncertain"]
}}
"""

PROMPTS["jitai_final_summary"] = """
You are summarizing a caregiver-facing JITAI timeline for an interaction session.
Summarize patterns across the session without making clinical claims.
...
{{
  ...
  "caregiver_strengths": ["strength 1"],
  "follow_up_focus": ["focus 1", "focus 2"]
}}
"""
